...
cas_onzecyl=Radiobutton(fenetre, text="5 Cyl. circ.+Ellipse", variable=var_cas, value=2)
...
if ((Cas == 2) or (Cas == 3)):
    ...
    cx = nx/10; cy1 = ny/6; cy2= ny/3; cy3 = ny/2; cy4 = 2*ny/3; cy5 = 5*ny/6; r = ny/25; cx2 = cx + 8*r; deccy = ny/24; relips = ny/10
    ...
    if(Cas == 2):
        obstacle = fromfunction(lambda x,y: (((x-cx)**2+(y-cy1)**2 <r**2)\
                                         ...
                                         | (0.05*(x*cos(theta)+y*sin(theta)-cxelips)**2+(y*cos(theta)+ x*sin(theta)-cyelips)**2 <relips**2)\
                                         | (y < 5) | (y > 355)), (nx,ny))
    if(Cas == 3):
        obstacle = fromfunction(lambda x,y: (((x-cx)**2+(y-cy1)**2 <r**2)\
                                         | ((x-cx)**2+(y-cy2)**2 <r**2) | ((x-cx)**2+(y-cy3)**2 <r**2)\
...
if (Cas == 1):
    ...
    Tx1 = 200; Tx2 = 220; Tx3 = Tx2 + ltpx; Ry1 = 180 - Ray; Ry2 = 180 + Ray; Ry3 = Ry1 - 4; Ry4 = Ry2 - 4; Ty2 = 4; Ty3 = 356 # Obstacle
    #r = Tx3-Tx2 # Longueur du tube en pixels ####
    r = Ry2-Ry1 # Internal tube diameter in pixels
    #obstacle = fromfunction(lambda x,y: ((((x <= Tx2)&( x >= Tx1))&((y <= Ry1)|(y >= Ry2)))|((((y <= Ry1)&(y >= Ry3))|((y <= Ry2)&(y >= Ry4)))&((x >= Tx1)&(x < Tx3))))|((((x<=Tx1)&(y<=Ty2))|((x<=Tx1)&(y>=Ty3)))) , (nx,ny))
    obstacle = fromfunction(lambda x,y: ((((x < Tx2)&( x > Tx1))&((y < Ry1)|(y > Ry2)))|((((y < Ry1)&(y > Ry3))|((y < Ry2)&(y > Ry4)))&\
...
for time in range(maxIter):
    ...
    #u[:,0,:] =vel[:,0,:]*(1. + AmpHP * sin(time * omega0helmholtz)) # Left wall: vitesse oscillante fréquence résonnance?
